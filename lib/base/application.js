...
Gensokyo.prototype.start = function() {
    ...
    if(this.useMetrics && this.option.metrics.riemann) {
        ...
        this.riemann = Riemann.createClient({
            host: this.option.metrics.riemann.host,
            port: this.option.metrics.riemann.port,
            transport: this.option.metrics.riemann.transport || "udp"
        });
        ...
        this.riemann.on("error", function(err) {
            console.error("An error occurred on Gensokyo Riemann.");
            console.error(err);
