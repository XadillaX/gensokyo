...
var Gensokyo = function(option) {
    ...
    if(this.option.metric && this.option.metric.reportPort) {
        this.useMetric = true;
        this.metricsServer = new Metrics.Server(this.option.metric.reportPort);
    }
...
Gensokyo.prototype.sendToRiemann = function() {
    // ask for all routers
    var result = this.metricsServer.report.summary();
    for(var $module in result) {
        if(!result.hasOwnProperty($module)) continue;
        var m = result[$module];
        for(var $function in m) {
            if(!m.hasOwnProperty($function)) continue;

            var metric = m[$function];
            var duration = metric.duration;
            var rate = metric.rate;

            var ok = {};
            ok.p95 = duration.p95 || 0;
            ok.p99 = duration.p99 || 0;
            ok.p999 = duration.p999 || 0;
            ok.m1 = rate.m1 || 0;
            ok.m5 = rate.m5 || 0;
            ok.m15 = rate.m15 || 0;

            var identity = $module + " " + $function;
            for(var $type in ok) {
                if(!ok.hasOwnProperty($type)) continue;
                this.riemann.send(this.riemann.Event({
                    service: identity + " " + $type,
                    metric: ok[$type],
                    state: "ok",
                    host: this.option["class"] + "@" + this.option.host + ":" + this.option.port
                }));
            }
        }
    }
...
Gensokyo.prototype.start = function() {
    ...
    if(this.useMetric && this.option.metric.riemann) {
        this.useRiemann = true;
        this.riemann = Riemann.createClient({
            host: this.option.metric.riemann.host,
            port: this.option.metric.riemann.port,
            transport: this.option.metric.riemann.transport || "udp"
        });

        this.riemann.on("connect", function() {
            console.info("Riemann connected.");
        });

        // create riemann sender
        this.riemannSendTimer = setInterval(
            this.sendToRiemann.bind(this),
            this.option.metric.riemann.sendInterval || 5000);
    }
    ...
    this.server.listen(function() {
        ...
        function cleanBeforeExit(code) {
            if(self.riemannSendTimer) clearInterval(this.riemannSendTimer);
